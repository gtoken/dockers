...
# From here on $PGDATA doesn't seem to change Postgres data directory, which upsets me

sudo -u postgres /usr/lib/postgresql/9.3/bin/postgres -D $PGDATA -c config_file=/etc/postgresql/9.3/main/postgresql.conf >logfile 2>&1 &
...
sudo -u postgres /usr/lib/postgresql/9.3/bin/postgres -D $PGDATA -c config_file=/etc/postgresql/9.3/main/postgresql.conf
